...
import SalesInvoiceSnViewModal from '../../../../components/UI/Modal/SerialNumber/SalesInvoice/SalesInvoiceSnViewModal'

import EditSalesInvoiceSerialNumModel from '../../../../components/UI/Modal/SerialNumber/SalesInvoice/EditSalesInvoiceSerialNumModel'

class SalesInvoiceSN extends Component {
  state={
    prefix:null,
    suffix:null,
    start_number:null,
  ...
  loadSalesInvoiceNumber=()=>{
    axios.get('masters/serial-number/').then(
      res=>{
        if(res.data.filter(item=>item.type=== "SI").length>0) {
          this.setState({
            serialNumObj:res.data.filter(item=>item.type === 'SI')[0],
            prefix:res.data.filter(item=>item.type === 'SI')[0].prefix,
            suffix:res.data.filter(item=>item.type === 'SI')[0].suffix,
            start_number:res.data.filter(item=>item.type === 'SI')[0].start_number,
...
editModal=()=>{
  return(
    <EditSalesInvoiceSerialNumModel
