...
import DebitNoteSnViewModal from '../../../../components/UI/Modal/SerialNumber/DebitNote/DebitNoteSnViewModal'

import EditDebitSerialNumModel from '../../../../components/UI/Modal/SerialNumber/DebitNote/EditDebitSerialNumModel'
...
class CredtiNoteSN extends Component {
  ...
  loadDebitNoteNumber=()=>{
    axios.get('masters/serial-number/').then(
      res=>{
        if(res.data.filter(item=>item.type=== "DN").length>0) {
          this.setState({
            serialNumObj:res.data.filter(item=>item.type === 'DN')[0],
            prefix:res.data.filter(item=>item.type === 'DN')[0].prefix,
            suffix:res.data.filter(item=>item.type === 'DN')[0].suffix,
            start_number:res.data.filter(item=>item.type === 'DN')[0].start_number,
