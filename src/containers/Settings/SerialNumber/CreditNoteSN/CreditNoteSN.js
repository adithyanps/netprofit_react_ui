...
import CreditNoteSnViewModal from '../../../../components/UI/Modal/SerialNumber/CreditNote/CreditNoteSnViewModal'

import EditCreditSerialNumModel from '../../../../components/UI/Modal/SerialNumber/CreditNote/EditCreditSerialNumModel'

class CredtiNoteSN extends Component {
  state={
    prefix:null,
    suffix:null,
    start_number:null,
  ...
  loadCreditNoteNumber=()=>{
    axios.get('masters/serial-number/').then(
      res=>{
        if(res.data.filter(item=>item.type=== "CN").length>0) {
          this.setState({
            serialNumObj:res.data.filter(item=>item.type === 'CN')[0],
            prefix:res.data.filter(item=>item.type === 'CN')[0].prefix,
            suffix:res.data.filter(item=>item.type === 'CN')[0].suffix,
            start_number:res.data.filter(item=>item.type === 'CN')[0].start_number,
