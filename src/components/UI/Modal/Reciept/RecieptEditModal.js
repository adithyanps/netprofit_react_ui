...
class RecieptEdit extends React.Component {
  state={
    serial_no:0,
    accountList:[],
    selectedAcnt:null,
  ...
  componentWillMount(){
    this.setState({
      ...
      debitJrnlItem:this.props.formData.journal_entry.debitJrnlItem,
      creditJrnlItem:this.props.formData.journal_entry.creditJrnlItem,
      ...
      total:this.props.formData.journal_entry.debitJrnlItem.debit_amount
    })
    let creditJrnlItem = this.props.formData.journal_entry.creditJrnlItem
    let debitJrnlItem = this.props.formData.journal_entry.debitJrnlItem
    // this.setState({total:debitJrnlItem.debit_amount})
    console.log(creditJrnlItem[0].id)
    let accntName=this.props.accountList.filter(item=>item.name===creditJrnlItem[0].account)
  ...
  handlePartnerChange= (idx) => (evt) => {
    const newholder = this.state.creditJrnlItem.map((element,sidx) => {
      if (idx !== sidx) {
        ...
        return { ...element, partner:evt.target.value};
      }
    });
  ...
  submitDataHandler=(e)=>{
    console.log(this.state.creditJrnlItem)
    console.log(this.state.debitJrnlItem)

    this.props.formData.journal_entry.creditJrnlItem = this.state.creditJrnlItem
    ...
    // delete this.props.formData.journal_entry.debitJrnlItem
    // delete this.props.formData.journal_entry.creditJrnlItem
            ...
            {this.state.creditJrnlItem.map((shareholder, idx) => (
              <div className="row-wrapper">
                      ...
                      <div>
                        ...
                        <option>{shareholder.partner}</option>
